 ...
 */
export async function fetchPRDiff(prInfo) {
    ...
    try {
        ...
        // Get PR details
        const prResponse = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}?api-version=7.1`, { headers });
        if (!prResponse.ok) {
        ...
        const iterResponse = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}/iterations?api-version=7.1`, { headers });
        let changeEntries = [];
        if (iterResponse.ok) {
            ...
            if (iterations.length > 0) {
                ...
                const changesResponse = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}/iterations/${latestIteration.id}/changes?api-version=7.1`, { headers });
        ...
        if (changeEntries.length === 0) {
            const commitsResponse = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}/commits?api-version=7.1`, { headers });
            if (commitsResponse.ok) {
                const commitsData = await commitsResponse.json();
                const commits = commitsData.value || [];
                for (const commit of commits) {
                    const commitChangesResponse = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/commits/${commit.commitId}/changes?api-version=7.1`, { headers });
        ...
        for (const entry of filesToProcess) {
            ...
            try {
                ...
                if (changeType === 'Delete') {
                    ...
                    // Fetch file content
                    const contentResponse = await fetch(`${apiBase}/git/repositories/${repoId || prInfo.repository}/items?path=${encodeURIComponent(path)}&versionDescriptor.version=${encodeURIComponent(sourceBranch)}&versionDescriptor.versionType=branch&includeContent=true&api-version=7.1`, { headers });
                    if (contentResponse.ok) {
                        ...
                        if (contentData.content) {
                            ...
                            if (changeType === 'Add') {
                                ...
                                const targetResponse = await fetch(`${apiBase}/git/repositories/${repoId || prInfo.repository}/items?path=${encodeURIComponent(path)}&versionDescriptor.version=${encodeURIComponent(targetBranch)}&versionDescriptor.versionType=branch&includeContent=true&api-version=7.1`, { headers });
...
function generateSimpleDiff(oldContent, newContent, maxChars) {
 ...
 */
export async function findBereanComments(prInfo) {
    const pat = getAzureDevOpsPATFromPipeline();
    if (!pat) {
        return [];
    }
    try {
        ...
        const response = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}/threads?api-version=7.1`, { headers });
        if (!response.ok) {
            return [];
        }
        const data = await response.json();
        const bereanComments = [];
 ...
 */
function extractReviewedCommits(content) {
 ...
 */
export function addReviewedCommitsTag(comment, commitIds) {
 ...
 */
export async function getPRCommits(prInfo) {
    const pat = getAzureDevOpsPATFromPipeline();
    if (!pat) {
        return [];
    }
    try {
        ...
        const response = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}/commits?api-version=7.1`, { headers });
        if (!response.ok) {
            return [];
        }
        const data = await response.json();
 ...
 */
export function shouldIgnorePR(description) {
    if (!description)
        return false;
 ...
 */
export async function updatePRComment(prInfo, threadId, commentId, newContent) {
    const pat = getAzureDevOpsPATFromPipeline();
    if (!pat) {
    ...
    try {
        ...
        const response = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}/threads/${threadId}/comments/${commentId}?api-version=7.1`, {
            method: 'PATCH',
            headers,
 ...
 */
export async function postPRComment(prInfo, comment) {
    const pat = getAzureDevOpsPATFromPipeline();
    if (!pat) {
        return { success: false, error: 'Azure DevOps PAT not configured' };
    }
    try {
        ...
        const response = await fetch(`${apiBase}/git/repositories/${prInfo.repository}/pullRequests/${prInfo.pullRequestId}/threads?api-version=7.1`, {
            method: 'POST',
            headers,
 ...
 */
export async function postInlineComment(prInfo, filePath, line, content) {
    const pat = getAzureDevOpsPATFromPipeline();
    if (!pat) {
        return { success: false, error: 'Azure DevOps PAT not configured' };
    }
 ...
 */
export async function postInlineComments(prInfo, comments) {
    const results = { success: 0, failed: 0, errors: [] };
    for (const comment of comments) {
        const result = await postInlineComment(prInfo, comment.filePath, comment.line, comment.content);
        if (result.success) {
            results.success++;
        }
        else {
